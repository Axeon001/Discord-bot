...
async def help_commands(ctx):
    ...
    embed.add_field(
        ...
        value="Добавляет указанное количество очков указанному пользователю. Если пользователь не указан, очки добавляются отправителю. Эти очки можно использовать через команду `!add_points` (администратор).",
        inline=False
    )
    embed.add_field(
        name="!help_commands",
